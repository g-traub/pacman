...
const animate = (element, direction) => {
  if (nextBox && nextBox.classList.contains("empty")) {
    if (
      nextBox.classList.contains("dot") ||
      nextBox.classList.contains("big-dot")
    ) {
      score += 10;
    ...
    if (i < 100) {
      switch (direction) {
        case "right":
          ...
          nextBox = document.querySelector(
            `.box[data-row='${element.dataset.row}'][data-column='${parseInt(
        ...
        case "down":
          ...
          nextBox = document.querySelector(
            `.box[data-row='${parseInt(element.dataset.row) +
              1}'][data-column='${parseInt(element.dataset.column)}']`
          );
          break;
        case "up":
          element.style.transform = `translateY(-${i}%)`;
      ...
      if (nextTurn) {
        switch (nextTurn) {
          case "right":
            turnTest = document.querySelector(
              `.box[data-row='${nextBox.dataset.row}'][data-column='${parseInt(
                nextBox.dataset.column
              ) + 1}']`
            );
            break;
          case "left":
          ...
          case "down":
            turnTest = document.querySelector(
              `.box[data-row='${parseInt(nextBox.dataset.row) +
                1}'][data-column='${parseInt(nextBox.dataset.column)}']`
            );
            break;
          case "up":
            turnTest = document.querySelector(
        ...
        if (turnTest && turnTest.classList.contains("empty")) {
          console.log(nextTurn);
          console.log(turnTest);
          console.log(element);
...
const keydownHandler = e => {
  ...
  switch (e.key) {
    case "ArrowRight":
      turnNextBox = document.querySelector(
        `.box[data-row='${presentBox.dataset.row}'][data-column='${parseInt(
          presentBox.dataset.column
        ) + 1}']`
      );
      if (
        turnNextBox &&
        turnNextBox.classList.contains("empty") &&
        e.key.substring(5).toLowerCase() !== direction
      ) {
        cancelAnimationFrame(animation);
        nextBox = turnNextBox;
    ...
    case "ArrowUp":
      turnNextBox = document.querySelector(
        `.box[data-row='${parseInt(presentBox.dataset.row) -
          1}'][data-column='${parseInt(presentBox.dataset.column)}']`
      );
      if (
        turnNextBox &&
        turnNextBox.classList.contains("empty") &&
